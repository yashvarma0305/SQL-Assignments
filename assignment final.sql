...
SELECT customerNumber, customerName, CASE WHEN country IN ('USA', 'Canada') THEN 'North America' WHEN country IN ('UK', 'France', 'Germany') THEN 'Europe' ELSE 'Other' END AS CustomerSegment FROM customers;
...
SELECT MONTHNAME(paymentDate) AS monthName, COUNT(*) AS totalPayments FROM Payments GROUP BY monthName HAVING totalPayments > 20 ORDER BY totalPayments DESC;
...
COUNT(orders.ordernumber) AS order_count FROM Customers JOIN Orders ON customers.customernumber=orders.customernumber
...
SELECT e.FullName AS ManagerName, m.FullName AS EmployeeName FROM Project as e LEFT JOIN Project as m ON m.EmployeeID = e.ManagerID;
use classocmodels;
...
with orderswithmonthyear as (select ordernumber,Year(orderdate) as Order_year,
...
ProductsAboveAvg AS (
    ...
    FROM 
        Products
    WHERE 
        buyPrice > (SELECT avgBuyPrice FROM AverageBuyPrice)
    GROUP BY 
        productLine)

SELECT productLine,productCount FROM ProductsAboveAvg;
drop table emp_eh;
CREATE TABLE Emp_EH (
...
CREATE DEFINER=root@localhost PROCEDURE InsertIntoEmp_EH (IN p_EmpName VARCHAR (100), IN p_EmailAddress VARCHAR (100))
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        ROLLBACK;
        SELECT 'Error occurred' AS ErrorMessage;
